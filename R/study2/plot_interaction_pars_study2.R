...
plot_interaction_pars_study2 <- function(study2_fit2_social,
  ...
  extract_fun <- function(study2_fit2) {
    # get model summary
    s <- summary(study2_fit2)
  ...
  out <-
    bind_rows(
      extract_fun(study2_fit2_social),
      extract_fun(study2_fit2_socialskills),
      extract_fun(study2_fit2_impactothers),
      extract_fun(study2_fit2_consequences),
      extract_fun(study2_fit2_intrinsiceffort),
      extract_fun(study2_fit2_extrinsiceffort)
      ) %>%
    mutate(
  ...
  # save
  ggsave(
    filename = "plots/study2_interaction_pars.pdf",
    plot = out,
    height = 6,
    width = 6
  )
