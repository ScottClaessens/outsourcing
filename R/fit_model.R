...
fit_model <- function(data) {
  ...
  brm(
    ...
    prior = c(
      ...
      prior(exponential(2), class = sd, resp = social),
      prior(exponential(2), class = sd, resp = socialskills),
      prior(exponential(2), class = sd, resp = impactothers),
      prior(exponential(2), class = sd, resp = consequences),
      prior(exponential(2), class = sd, resp = intrinsiceffort),
      prior(exponential(2), class = sd, resp = extrinsiceffort),
      prior(lkj(3), class = cor, group = id),
      prior(lkj(3), class = cor, group = task)
    ),
    chains = 4,
    cores = 4,
    seed = 2113
