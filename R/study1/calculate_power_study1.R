# function to run simulated power analysis for study 1
calculate_power_study1 <- function(n = 100, effect_size = -0.5, nsims = 100) {
  # function to simulate data
  simulate_data <- function(n) {
    tasks <- c()
    for (i in 1:n) {
      tasks <- c(tasks, sample(paste0("task", 1:20), size = 6))
    }
    tibble(
      id = rep(1:n, each = 6),
      condition = rep(
        sample(c("control", "treatment"), size = n, replace = TRUE),
        each = 6
      ),
      task = tasks,
      rating = sample(1:7, size = n*6, replace = TRUE)
    )
  }
  # get dummy dataset
  d <- simulate_data(n)
  ...
  prior_fit <-
    brm(
      formula = rating ~ 1 + condition + (1 | id) + (1 + condition | task),
      data = d,
      family = cumulative,
      prior = c(
        # this is the effect size
        prior_string(paste0("normal(", effect_size, ", 0.05)"), 
                     class = "b", coef = "conditiontreatment"),
        # other priors
        prior(normal(-2.0, 0.05), class = Intercept, coef = 1),
        prior(normal(-0.6, 0.05), class = Intercept, coef = 2),
        prior(normal( 0.2, 0.05), class = Intercept, coef = 3),
        prior(normal( 1.0, 0.05), class = Intercept, coef = 4),
        prior(normal( 2.1, 0.05), class = Intercept, coef = 5),
        prior(normal( 3.0, 0.05), class = Intercept, coef = 6),
        prior(normal( 1.0, 0.05), class = sd),
