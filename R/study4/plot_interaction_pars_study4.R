...
plot_interaction_pars_study4 <- function(study4_fit2_social,
  ...
  extract_fun <- function(study4_fit2) {
    # get model summary
    s <- summary(study4_fit2)
    # interaction parameters only
    s$fixed[str_detect(rownames(s$fixed), ":"),] %>%
      rownames_to_column() %>%
      mutate(
        rowname = str_replace(rowname, "AI_", ""),
        rowname = str_replace(rowname, "Standard_", "Standard-"),
        rowname = str_replace(rowname, "Personalised_", "Personalised-")
        ) %>%
      separate(
        rowname,
  ...
  out <-
    bind_rows(
      extract_fun(study4_fit2_social),
      extract_fun(study4_fit2_socialskills),
      extract_fun(study4_fit2_impactothers),
      extract_fun(study4_fit2_consequences),
      extract_fun(study4_fit2_intrinsiceffort),
      extract_fun(study4_fit2_extrinsiceffort)
      ) %>%
    mutate(
  ...
  # save
  ggsave(
    filename = "plots/study4_interaction_pars.pdf",
    plot = out,
    height = 6,
    width = 6
  )
