# function to plot treatment effects by task in study 5
plot_treatments_by_task_study5 <- function(study5_data, study5_fit1) {
  ...
  extract_diffs <- function(resp) {
    ...
    d %>%
      ...
      transmute(
        ...
        Task = task,
        Tool_BadReason  = list(AI_Tool_Bad  - Control),
        Tool_GoodReason = list(AI_Tool_Good - Control),
        Full_BadReason  = list(AI_Full_Bad  - Control),
        Full_GoodReason = list(AI_Full_Good - Control)
      ) %>%
      pivot_longer(
        cols = Tool_BadReason:Full_GoodReason,
        names_to = "Effect",
        values_to = "post"
      ) %>%
      separate(
      ...
      rowwise() %>%
      mutate(
        Reason = ifelse(Reason == "BadReason", "Bad reason", "Good reason"),
        Estimate = mean(post),
        Est.Error = sd(post),
  ...
  diffs <-
    ...
    mutate(
      ...
      Reason = factor(Reason, levels = c("Bad reason", "Good reason")),
  ...
  # save
  ggsave(
    filename = "plots/study5_treatments_by_task.pdf",
    plot = p,
    width = 8.5,
    height = 4
  )
