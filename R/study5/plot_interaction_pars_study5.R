...
plot_interaction_pars_study5 <- function(study5_fit2_social,
  ...
  extract_fun <- function(study5_fit2) {
    # get model summary
    s <- summary(study5_fit2)
    # interaction parameters only
    s$fixed[str_detect(rownames(s$fixed), ":"),] %>%
      rownames_to_column() %>%
      mutate(
        rowname = str_replace(rowname, "AI_", ""),
        rowname = str_replace(rowname, "Tool_", "Tool-"),
        rowname = str_replace(rowname, "Full_", "Full-")
        ) %>%
      separate(
        rowname,
  ...
  out <-
    bind_rows(
      extract_fun(study5_fit2_social),
      extract_fun(study5_fit2_socialskills),
      extract_fun(study5_fit2_impactothers),
      extract_fun(study5_fit2_consequences),
      extract_fun(study5_fit2_intrinsiceffort),
      extract_fun(study5_fit2_extrinsiceffort)
      ) %>%
    mutate(
  ...
  # save
  ggsave(
    filename = "plots/study5_interaction_pars.pdf",
    plot = out,
    height = 6,
    width = 6
  )
