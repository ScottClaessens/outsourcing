# function to fit model 1 to study 5 data
fit_study5_model1 <- function(study5_data) {
  ...
  bf6 <- bf(as.formula(paste0("care",        f)), family = cumulative)
  # fit model
  brm(
    ...
    prior = c(
      ...
      prior(exponential(4), class = sd, resp = care),
